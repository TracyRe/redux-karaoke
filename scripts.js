...
3: "I can't seem to face up to the facts, I'm tense and nervous and I can't relax, I can't sleep 'cause my bed's on fire, Don't touch me I'm a real live wire, Psycho Killer Qu'est-ce que c'est, Fa-fa-fa-fa-fa-fa-fa-fa-fa-far better, Run run run run run run run away oh oh, Psycho Killer Qu'est-ce que c'est, Fa-fa-fa-fa-fa-fa-fa-fa-fa-far better, Run run run run run run run away oh oh oh, Yeah yeah yeah yeah!, You start a conversation you can't even finish it, You're talking a lot, but you're not saying anything, When I have nothing to say, my lips are sealed, Say something once, why say it again?, Psycho Killer Qu'est-ce que c'est, Fa-fa-fa-fa-fa-fa-fa-fa-fa-far better, Run run run run run run run away oh oh, Psycho Killer Qu'est-ce que c'est, Fa-fa-fa-fa-fa-fa-fa-fa-fa-far better, Run run run run run run run away oh oh oh, Yeah yeah yeah yeah!".split(', '),
...
const renderLyrics = () => {
  ...
  if (store.getState().currentSongId) {
    const currentLine = document.createTextNode(store.getState().songsById[store.getState().currentSongId].songArray[store.getState().songsById[store.getState().currentSongId].arrayPosition]);
      ...
      const selectSongMessage = document.createTextNode("Select a song from the menu");
...
const renderSongs = () => {
  ...
  for (const songKey in songsById) {
    ...
    const songArtist = document.createTextNode(' by ' + song.title);
